...
stadium <- read_csv("stadium_jsutteamname.csv")
...
2) 
<br>
3) Which has a stronger correlation to wins, batting or pitching?
<br>
4) Is ERA play a factor into a teams performance in the regular season
<br>
5) How much of an effect does ERA play in the post season, specifically for World Series Wins?
<br>
...
```{r}
graph1 <- ggplot(team_data_ws, aes(x = as.factor(cluster), y = win_pct)) +
  geom_boxplot() +
...
```{r}
graph3 <- ggplot(team_data_ws, aes(x = as.factor(cluster), y = ERA)) +
  geom_boxplot() +
...
```{r grpah4}
graph4 <- ggplot(team_data_ws, aes(x = as.factor(cluster), y = total_SO)) +
  geom_boxplot() +
...
team_data_map <- merge(team_data_ws, stadium, by = "teamID")
cluster_colors <- c("blue", "red")

leaflet(team_data_map) %>%
  addTiles() %>%
  addCircleMarkers(
    lng = ~lng,
    lat = ~lat,
    popup = ~paste0("Team: ", teamID, "<br>Cluster: ", cluster),
